...
from models import AutoEncoder3D
# from torchmetrics import StructuralSimilarityIndexMeasure
# from torchmetrics.image import StructuralSimilarityIndexMeasure
from torchmetrics.image import StructuralSimilarityIndexMeasure as SSIM
from torchmetrics.image import PeakSignalNoiseRatio as PSNR

class AutoEncoderSolver:
    def __init__(self, train_loader, test_loader, config):
        ...
        self.autoencoder_lr = config['training']['autoencoder_lr']
        ...
        # Directories.
        self.log_dir = config['directories']['autoencoder_log_dir']
        self.model_save_dir = config['directories']['autoencoder_model_save_dir']
        self.sample_dir = config['directories']['autoencoder_sample_dir']
        self.result_dir = config['directories']['autoencoder_result_dir']
    ...
    #
    def _load_checkpoint(self, resume_iters):
        pass
    ...
    def build_model(self):
        self.model = AutoEncoder3D(self.g_conv_dim, self.g_repeat_num).to(self.device)
        self.optimizer = optim.Adam(self.model.parameters(), self.autoencoder_lr, [self.beta1, self.beta2])
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.num_iters):
            ...
            if (epoch+1) % self.lr_update_step == 0 and (epoch+1) > (self.num_iters - self.num_iters_decay):
                lr -= (self.autoencoder_lr / float(self.num_iters_decay))
                self.update_lr(lr)
    ...
    def _evaluate(self, dataloader):
        ...
        with torch.no_grad():
            for batch in dataloader:
                ...
                fake_MR = self.model(real_CT)
                loss_l1 = self.criterionL1(fake_MR, real_MR)
                
                losses['loss_l1'].append(loss_l1.item())
