...
class BugFree:
    ...
    def get_bug_free_str(self):
        try:
            with open(bugfree_img_path + self.img_name + '.bugfree', 'r') as f:
                return f.read()
        except:
            return "上天保佑，代码无BUG!"
    ...
    def get_bug_free_gradient_texts(self):
        ...
        gt = self.gradient_texts(bug_free_texts)
        return gt
...
bugfree_img_path = './utils/bugfree_img/' # 注意相对于工作目录的路径

def get_bug_free_str(img_name='buddha'):
    try:
        with open(bugfree_img_path + img_name + '.bugfree', 'r') as f:
            return f.read()
    except:
        return "上天保佑，代码无BUG!"
...
def get_bug_free_gradient_texts(img_name='buddha', start_color="FF7EC7", end_color="FFED46"):
    bug_free_texts = get_bug_free_str(img_name).splitlines()  # 按行分割
    gt = gradient_texts(bug_free_texts, start_color, end_color)
    return gt

def print_bug_free_gradient_texts(img_name='buddha', start_color="FF7EC7", end_color="FFED46"):
    gts = get_bug_free_gradient_texts(img_name, start_color, end_color)
    for gt in gts:
        print(gt)