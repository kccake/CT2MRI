...
class UpBlock3d(torch.nn.Module):
    def __init__(self, nc_down_stream, nc_skip_stream, nc_out, kernel_size, stride, padding, bias=True,
                 activation='relu', init_func='kaiming', use_norm=False, refine=False, use_resnet=False, use_add=False,
        ...
        if 'nc_inner' in kwargs:
            ...
            nc_inner = nc_out
        self.conv_0 = Conv3d(nc_down_stream + nc_skip_stream, nc_inner, kernel_size=kernel_size, stride=stride,
                             padding=padding, bias=bias, activation=activation, init_func=init_func, use_norm=use_norm,
                             use_resnet=use_resnet, **kwargs)
        self.conv_1 = None
        if refine:
            self.conv_1 = Conv3d(nc_inner, nc_inner, kernel_size=kernel_size, stride=stride, padding=padding, bias=bias,
                                 activation=activation, init_func=init_func, use_norm=use_norm, use_resnet=use_resnet,
                                 **kwargs)
        self.use_attention = use_attention
        if self.use_attention:
            self.attention_gate = AttentionGate3d(nc_down_stream, nc_skip_stream, nc_inner, use_norm=True,
                                                  init_func=init_func)
        self.up_conv = Conv3d(nc_inner, nc_out, kernel_size=kernel_size, stride=stride, padding=padding, bias=bias,
                              activation=activation, init_func=init_func, use_norm=use_norm, use_resnet=False, **kwargs)
        self.use_add = use_add
        if self.use_add:
            self.output = Conv3d(nc_out, 2, kernel_size=1, stride=1, padding=0, bias=bias, activation=None,
                                 init_func='zeros', use_norm=False, use_resnet=False)
...
class AttentionGate3d(torch.nn.Module):
    def __init__(self, nc_g, nc_x, nc_inner, use_norm=False, init_func='kaiming', mask_channel_wise=False):
        ...
        self.conv_g = Conv3d(nc_g, nc_inner, 1, 1, 0, bias=True, activation=None, init_func=init_func,
                             use_norm=use_norm, use_resnet=False)
        self.conv_x = Conv3d(nc_x, nc_inner, 1, 1, 0, bias=False, activation=None, init_func=init_func,
                             use_norm=use_norm, use_resnet=False)
        self.residual = nn.ReLU(inplace=True)
        self.mask_channel_wise = mask_channel_wise
        self.attention_map = Conv3d(nc_inner, nc_x if mask_channel_wise else 1, 1, 1, 0, bias=True, activation='sigmoid',
    ...
    def forward(self, g, x):
        ...
        if x_c.size(2) != g_size[2] and x_c.size(3) != g_size[3] and x_c.size(4) != g_size[4]:
            x_c = F.interpolate(x_c, (g_size[2], g_size[3], g_size[4]), mode=up_sample_mode, align_corners=align_corners)
        combined = self.residual(g_c + x_c)
        alpha = self.attention_map(combined)
        if not self.mask_channel_wise:
            alpha = alpha.repeat(1, x_size[1], 1, 1, 1)
        alpha_size = alpha.size()
        if alpha_size[2] != x_size[2] and alpha_size[3] != x_size[3] and alpha_size[4] != x_size[4]:
            alpha = F.interpolate(x, (x_size[2], x_size[3], x_size[4]), mode=up_sample_mode, align_corners=align_corners)
        return alpha * x
...
class ResnetTransformer3d(torch.nn.Module):
    def __init__(self, dim, n_blocks, init_func):
        ...
        for i in range(n_blocks):  # add ResNet blocks
            model += [
                ResnetBlock3d(dim, padding_type='reflect', norm_layer=norm_layer, use_dropout=False,
                              use_bias=True)]
