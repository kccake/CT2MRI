...
class ResUnet(torch.nn.Module):
    ...
    def forward(self, img_a, img_b):
        ...
        while conv_num > (self.ndown_blocks - self.nup_blocks):
            ...
            x = F.interpolate(x, size=(s.size(2), s.size(3), s.size(4)), mode='trilinear')
            x = torch.cat([x, s], 1)
...
class Reg(nn.Module):
    def __init__(self, depth, height, width, in_channels_a, in_channels_b):
        super(Reg, self).__init__()
        init_func = 'kaiming'
        init_to_identity = True

        self.od, self.oh, self.ow = depth, height, width
    ...
    def get_identity_grid(self):
        ...
        zz = zz.unsqueeze(dim=0)
        yy = yy.unsqueeze(dim=0)
        xx = xx.unsqueeze(dim=0)
        identity = torch.cat((zz, yy, xx), dim=0).unsqueeze(0)
        return identity

    def forward(self, vol_a, vol_b, apply_on=None):
        deformations = self.offset_map(vol_a, vol_b)
        return deformations
