...
# local
from .layers2D import DownBlock2d, Conv2d, ResnetTransformer2d
...
class ResUnet2D(torch.nn.Module):
    def __init__(self, nc_a, nc_b, cfg, init_func, init_to_identity):
        ...
        for out_nf in nuf[cfg]:
            setattr(self, f'up_{conv_num}',
                    Conv2d(in_nf + skip_nf[f'down_{conv_num}'], out_nf, 3, 1, 1, bias=True, activation=act,
        ...
        if refine_output[cfg]:
            self.refine = nn.Sequential(ResnetTransformer2d(in_nf, 1, init_func),
                                        Conv2d(in_nf, in_nf, 1, 1, 0, use_resnet=False, init_func=init_func,
                                               activation=act, use_norm=False))
        else:
            self.refine = lambda x: x
        self.output = Conv2d(in_nf, 2, 3, 1, 1, use_resnet=False, bias=True,
...
class Reg2D(nn.Module):
    def __init__(self, height, width, in_channels_a, in_channels_b):
        ...
        self.offset_map = ResUnet2D(self.in_channels_a, self.in_channels_b, cfg='A', init_func=init_func, init_to_identity=init_to_identity).to(self.device)
